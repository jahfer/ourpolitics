...
module Translate = (M: Translatable) => {
  ...
  let react_string = (~language: language, item: M.t) => to_str(item, ~language)->React.string
}
...
module T = (M: Translatable) => {
  ...
  module WithLanguage = (LS: LanguageSelection) => {
    ...
    let to_str = item => T.to_str(~language=LS.language, item)

    let react_string = (item: M.t) => T.to_str(item, ~language=LS.language)->React.string
  }
}
